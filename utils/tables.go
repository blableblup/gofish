package utils

import (
	"context"
	"fmt"
	"gofish/logs"
	"strings"

	"github.com/jackc/pgx/v5/pgxpool"
)

func EnsureTableExists(pool *pgxpool.Pool, tableName string) error {
	// Check if the table exists by querying the information_schema.tables
	var exists bool
	err := pool.QueryRow(context.Background(), "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE lower(table_name) = lower($1))", tableName).Scan(&exists)
	if err != nil {
		return err
	}

	if !exists {
		logs.Logs().Info().Msgf("Table '%s' does not exist, creating...", tableName)
	}

	// Create the appropriate table if it doesn't exist
	if !exists {
		switch {
		case tableName == "fish":
			_, err := pool.Exec(context.Background(), fmt.Sprintf(`
				CREATE TABLE %s (
					fishid SERIAL PRIMARY KEY,
					chatid INT,
					fishtype VARCHAR,
					fishname VARCHAR,
					weight FLOAT,
					catchtype VARCHAR,
					player VARCHAR,
					playerid INT,
					date TIMESTAMP,
					bot VARCHAR,
					chat VARCHAR,
					url VARCHAR
				)
			`, tableName))
			if err != nil {
				return err
			}

		case tableName == "bag":
			// https://www.postgresql.org/docs/16/sql-createtable.html#SQL-CREATETABLE-PARMS-GENERATED-IDENTITY for the primary key
			_, err := pool.Exec(context.Background(), fmt.Sprintf(`
				CREATE TABLE %s (
					bagid INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
					bag VARCHAR [],
					player VARCHAR,
					playerid INT,
					date TIMESTAMP,
					bot VARCHAR,
					chat VARCHAR,
					url VARCHAR
				)
			`, tableName))
			if err != nil {
				return err
			}

		case tableName == "ambience":
			_, err := pool.Exec(context.Background(), fmt.Sprintf(`
				CREATE TABLE %s (
					ambientid INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
					ambience VARCHAR,
					ambiencename VARCHAR,
					location VARCHAR,
					sublocation VARCHAR,
					player VARCHAR,
					playerid INT,
					date TIMESTAMP,
					bot VARCHAR,
					chat VARCHAR,
					url VARCHAR
				)
			`, tableName))
			if err != nil {
				return err
			}

		case tableName == "ambienceinfo":
			_, err := pool.Exec(context.Background(), fmt.Sprintf(`
				CREATE TABLE %s (
					ambiencename VARCHAR PRIMARY KEY,
					ambience VARCHAR,
					location VARCHAR,
					sublocation VARCHAR,
					oldemojis VARCHAR []
				)
			`, tableName))
			if err != nil {
				return err
			}

		case tableName == "fishinfo":
			_, err := pool.Exec(context.Background(), fmt.Sprintf(`
				CREATE TABLE %s (
					fishname VARCHAR PRIMARY KEY,
					fishtype VARCHAR,
					oldemojis VARCHAR [],
					shiny VARCHAR [],
					tags VARCHAR []
				)
			`, tableName))
			if err != nil {
				return err
			}

		case tableName == "playerdata":
			_, err := pool.Exec(context.Background(), fmt.Sprintf(`
				CREATE TABLE %s (
					playerid SERIAL PRIMARY KEY,
					twitchid INT,
					name VARCHAR,
					oldnames  VARCHAR [],
					verified BOOLEAN,
					firstfishdate TIMESTAMP,
					firstfishchat VARCHAR
				)
			`, tableName))
			if err != nil {
				return err
			}

		case strings.HasPrefix(tableName, "tournaments"):
			_, err := pool.Exec(context.Background(), fmt.Sprintf(`
				CREATE TABLE %s (
					trnmid INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
					player VARCHAR,
					playerid INT,
					fishcaught INT,
					placement1 INT,
					totalweight FLOAT,
					placement2 INT,
					biggestfish FLOAT,
					placement3 INT,
					date TIMESTAMP,
					bot VARCHAR,
					chat VARCHAR,
					url VARCHAR
				)
			`, tableName))
			if err != nil {
				return err
			}

		default:
			return fmt.Errorf("unsupported table name: %s", tableName)
		}
	}

	if !exists {
		logs.Logs().Info().Msgf("Table '%s' created successfully", tableName)

	}

	return nil
}
