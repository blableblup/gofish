# made with chatgpt
# adds the fish someone caught in the current month to their all time count so i dont have to check every url since december 2022
...
async def main(renamed_chatters, cheaters, verified_players):
    ...
    # get the players old totalcount from another leaderboard
    old_count = {}
    ...
    fish_catch_count = {player: count + old_count.get(player, 0) for player, count in fish_catch_count.items() if count + old_count.get(player, 0) >= 100}

    # Add players who fished in the past to fish_catch_count
    for player in old_count:
        if player not in fish_catch_count and old_count[player] > 100:
            fish_catch_count[player] = old_count[player]
        ...
        for count, players in ranks.items():
            for player in players:
                ...
                if old_bot[player] == "supibot":
                    file.write(f"| {rank} {movement[player]}| {player}* | {count} |\n")
                else:
                    file.write(f"| {rank} {movement[player]}| {player} | {count} |\n")
            rank += len(players)
